			...
			try {
				try {
					...
					var status_code = (err is Informational ||
					                   ...
					                   err is Redirection ||
					                   err is ClientError ||
									   err is ServerError) ?  err :
						new ServerError.INTERNAL_SERVER_ERROR (err.message);

					// handle using a registered status handler
					if (this.status_handlers.contains (status_code.code)) {
						var stack = new Queue<Value?> ();
						stack.push_tail (status_code.message);
						if (this.perform_routing (this.status_handlers[status_code.code].head, req, res, stack)) {
							return;
						}
					}
