...
namespace VSGI.FastCGI {
	class StreamInputStream : InputStream, PollableInputStream {
		...
		public StreamInputStream (GLib.Socket socket, global::FastCGI.Stream stream) {
		...
		public PollableSource create_source (Cancellable? cancellable = null) {
			return new PollableSource.full (this, this.socket.create_source (IOCondition.IN, cancellable), cancellable);
		}
	...
	class StreamOutputStream : OutputStream, PollableOutputStream {
		...
		public PollableSource create_source (Cancellable? cancellable = null) {
			return new PollableSource.full (this, this.socket.create_source (IOCondition.OUT, cancellable), cancellable);
		}
		...
		public Request (HashTable<string, string> environment , InputStream base_stream) {
			...
			environment.foreach ((name, @value) => {
				if (name.has_prefix ("HTTP_")) {
					headers.append (name.substring (5).replace ("_", "-").casefold (), @value);
				}
			});
		}
	}
